  ...
  function () {
    ...
    (0, _createClass2["default"])(LinkInlineTool, [{
      ...
      value: function renderActions() {
        ...
        this.nodes.header = document.createElement('h6');
        this.nodes.header.appendChild(document.createTextNode('Link to Existing Page'));
        this.nodes.ul = document.createElement('ul');
        ...
        this.nodes.searchInput.addEventListener('keyup', function (event) {
          console.log(_this.nodes.searchInput.value);

          if (_this.nodes.searchInput.value.trim().length) {
            var filtertedList = _this.pageLinks.filter(function (each) {
              return each.name.toLocaleLowerCase().includes(_this.nodes.searchInput.value.trim());
            });

            _this.renderList(filtertedList);
          } else {
            _this.renderList(_this.pageLinks);
          }
        });
        ...
        this.nodes.input.addEventListener('keydown', function (event) {
          if (event.keyCode === _this.ENTER_KEY) {
            _this.enterPressed(event, false, '');
          }
        });
      ...
      value: function checkState(selection) {
        ...
        if (anchorTag) {
          ...
          if (hrefAttr !== 'null') {
            this.pageLinks.forEach(function (page, index) {
              if (page.url === hrefAttr) {
                _this2.linkElements[index].classList.add('selected-item');
              } else {
                _this2.linkElements[index].classList.remove('selected-item');
              }
            });
          } else {
      ...
      value: function renderList(list) {
        ...
        list.forEach(function (each) {
          ...
          li.addEventListener('click', function (event) {
            ...
            _this3.enterPressed(event, true, each.url);
          });

          _this3.nodes.ul.appendChild(li);

          _this3.linkElements.push(li);
        });
      }
    }, {
      key: "toggleActions",
      ...
      key: "enterPressed",
      value: function enterPressed(event, isExisting, data) {
        var value = '';
